...
function orderValues (values_array: number[]) {
    ...
    for (let j = 0; j <= values_array.length - 1; j++) {
        for (let k = 0; k <= values_array.length - j - 1 - 1; k++) {
            if (values_array[k] < values_array[k + 1]) {
                spare_value2 = values_array[k]
                values_array[k] = values_array[k + 1]
                values_array[k + 1] = spare_value2
                spare_card2 = global_hand_array[k]
                global_hand_array[k] = global_hand_array[k + 1]
                global_hand_array[k + 1] = spare_card2
            }
        }
    }
    for (let l = 0; l <= values_array.length - 1; l++) {
        next2 = values_array[l + 1]
        if (next2 != undefined) {
            if (values_array[l] == next2 + 1) {
                current2 += 1
                if (current2 > row2) {
                    row2 = current2
                }
            } else if (values_array[l] != next2) {
                current2 = 0
            }
            if (values_array[l] == next2) {
                h += 1
            } else {
                if (h > 1) {
                    card_groups2.push(h)
